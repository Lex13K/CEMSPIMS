...
def test_cli_run_ingest_with_absolute_paths(tmp_path) -> None:
    ...
    assert code == 0
    assert (shared_interim / "ingest_manifest.json").is_file()
    assert (shared_interim / "returns_panel.parquet").is_file()
    assert (shared_interim / "targets.parquet").is_file()
    assert (shared_interim / "targets_manifest.json").is_file()
